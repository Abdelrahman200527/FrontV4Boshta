...
const Grades = () => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      await downloadGradesTemplateAction();
    ...
    try {
      ...
      fileInput.onchange = async (e) => {
        ...
        try {
          ...
          const result = await bulkUploadGradesAction(formData);
          ...
          if (result.success) {
            ...
            if (successCount > 0) {
              notifySuccess(`تم استيراد ${successCount} صف بنجاح`);
            }
            if (errorCount > 0) {
              notifyError(`فشل استيراد ${errorCount} صف`);
            }
  ...
  const saveMutation = useApiMutation(
    ({ id, payload }) =>
      id ? updateGradeInfo(id, payload) : createNewGrade(payload),
    {
